...
REGISTER_COMMAND(
		[](sw::core::World& world, sw::core::io::CommandParser& parser)
		{
			parser.add<sw::features::commands::March>(
					[&world](auto command)
					{
						...
						world.getEvents().event(
								world.getTick(),
								sw::features::events::MarchStarted{
										command.unitId, currentPos.x, currentPos.y, targetPos.x, targetPos.y});
					});
		})
