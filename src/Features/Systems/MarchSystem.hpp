...
{
	static std::vector<core::domain::Position> getOccupiedCells(
			core::World& world, uint32_t unitId, core::domain::Position anchor)
	{
		...
		for (const auto& [id, _] : occupiers)
		{
			...
			for (const auto& selfCell : selfCells)
			{
				if (std::any_of(
							targetCells.begin(),
							targetCells.end(),
							[&](const auto& targetCell) { return areCellsNeighbors(selfCell, targetCell); }))
				{
					isNeighbor = true;
					break;
				}
	...
	static void execute(core::World& world, intents::MarchIntent& intent)
	{
		...
		world.getEvents().event(
				world.getTick(), core::events::UnitMoved{intent.unitId, intent.posTo.x, intent.posTo.y});
	}
	...
	static void onAfterMove(core::World& world, intents::MarchIntent& intent)
	{
		...
		{
			world.getEvents().event(
					world.getTick(), features::events::MarchEnded{intent.unitId, intent.posTo.x, intent.posTo.y});
			targets.erase(it);
		}
	}
}
